...
# We add `setup` as a fixture to this test so that it isn't
# run before `test_case` is assigned.
@pytest.mark.parametrize('test_case', [0, 1, 2])
@pytest.mark.slow
def test_init_LP_RP_iterative(setup, test_case):
    H, psi, E, name, N_cycles, cycle_indices, square_of_H, square_of_psi = setup
