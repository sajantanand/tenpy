...
from .site import group_sites, DoubledSite
...
class MPS(BaseMPSExpectationValue):
    ...
    def compress_dmt_canonical(self, trunc_par, dmt_par, move_right=True,
        ...
        the chain as we truncate. DMT truncates on bond i and sets tensor i in A form and tensor i+1 in B form.
        To progress to bond i+1 (for right sweeping), we need either tensor i+1 or i+2 to be the OC with other
        tensors pointing in.
        ...
        trunc_par : dict
            Parameters for truncation, see :cfg:config:`truncation`.
        """
        # We could run DMT on grouped sites.
        # assert np.alltrue([type(s) == DoubledSite for s in self.sites]), "MPS needs to be a doubled MPS (with doubled sites) to use DMT."
...
class NonTrivialMPSEnvironment(BaseEnvironment):
    ...
    def init_LP(self, i, start_env_sites=0):
        ...
        # Won't be true!
        #leg_ket.test_equal(leg_bra)
        combined_leg_ket = npc.LegPipe([leg_bra, leg_ket.conj()], leg_ket.qconj)
